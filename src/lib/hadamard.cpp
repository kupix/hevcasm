...
void hevcasm_populate_hadamard_satd(hevcasm_table_hadamard_satd<Sample> *table, hevcasm_code code)
{
	...
	*hevcasm_get_hadamard_satd(table, 3) = 0;
	...
	{
		*hevcasm_get_hadamard_satd(table, 1) = compute_sat_c_opt_2x2<Sample>;
		*hevcasm_get_hadamard_satd(table, 2) = compute_sat_c_opt_4x4<Sample>;
		*hevcasm_get_hadamard_satd(table, 3) = compute_sat_c_opt_8x8<Sample>;
	}

#if USE_HM_DERIVED
	if (buffer.isa & HEVCASM_C_OPT)
	{
		*hevcasm_get_hadamard_satd(table, 1) = compute_satd_c_ref<2, Sample>;
		*hevcasm_get_hadamard_satd(table, 2) = compute_satd_c_ref<4, Sample>;
		*hevcasm_get_hadamard_satd(table, 3) = compute_satd_c_ref<8, Sample>;
	}
#endif
